...
class Visualization(object):
    ...
    def set_param(self):
        ...
        self.viz_output_path = (
            f"{dir_name}/{self.video_name}_{args.detector_2d}.mp4".replace(
                "source_video",
                args.architecture.replace(",", "")
                + "_scale2D_{:0>3d}".format(int(args.pose2dscale * 10)),
            )
        )
        # prepare folder
        ...
        self.joints_left, self.joints_right = list([4, 5, 6, 10, 11, 12]), list(
            [1, 2, 3, 13, 14, 15]
        )
    ...
    def get_keypoints(self):
        ...
        tmp_npz_path = args.viz_video_path.replace(".mp4", "_det2D.npz").replace(
            "source_video", "det2D_" + args.detector_2d
        )
        ...
        self.keypoints_imgnorm = normalize_screen_coordinates(
            self.keypoints_imgunnorm[..., :2], w=args.video_width, h=args.video_height
        )
    ...
    def keypoint_smoothing(self, keypoints):
        ...
        window_length = 5
        polyorder = 2
        out = scipy.signal.savgol_filter(
            x,
            window_length,
            polyorder,
            deriv=0,
    ...
    def visalizatoin(self, anim_output):
        ...
        render_animation(
            ...
            size=args.viz_size,
            input_video_path=viz_video_path,
            viewport=(args.video_width, args.video_height),
    ...
    def save_3d_prediction(self, anim_output):
        tmp_anim_output = {}
        for tmp_key in anim_output:
            prediction = anim_output[tmp_key] * 1.0
            if args.add_trajectory:
                # add root trajectory
                prediction -= prediction[:, :1, :]
                prediction += self.root_trajectory
            tmp_anim_output[tmp_key] = prediction * 1.0
        # save 3D joint points
        tmp_pkl_path = args.viz_video_path.replace(".mp4", "_pred3D.pkl").replace(
            "source_video", "pred3D_pose"
        )  # rename to save
        mkd(tmp_pkl_path)
...
if __name__ == "__main__":
    ...
    for args.architecture in ["3,3,3"]:
        ...
        for args.pose2dscale in [1]:
            ...
            eval_video_list = glob.glob("./input/*.mp4")
